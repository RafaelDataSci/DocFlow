...
def run_word_processing(values):
    """Run Word Processing task."""
    origin_folder = values["-WP_ORIGIN-"]
    dest_folder = values["-WP_DEST-"]
    replacement_pairs = [list(row) for row in values["-TABLE-"]]

    if not replacement_pairs:
        log("❌ No replacement pairs defined!")
        return
...
while True:
    ...
    # --- Word Processing Tab Events ---
        ...
        if old_text and new_text:
            ...
            if [old_text, new_text] not in current_table:
                current_table.append([old_text, new_text])
    ...
    # --- Task Execution Events ---
        ...
        if not values["-TABLE-"]:
            ...
            threading.Thread(target=run_word_processing, args=(values,), daemon=True).start()
