...
def run_word_processing(values):
    ...
    replacement_pairs = window["-TABLE-"].get()  # Get data directly from table
...
while True:
    ...
    # --- Task Execution Events ---
        ...
        # Capture the replacement pairs HERE (in the main thread)
        replacement_pairs = window["-TABLE-"].get()
        if not replacement_pairs:
            log("❌ Add replacement pairs first!")
        else:
            # Pass the pairs directly to the thread
            threading.Thread(
                target=run_word_processing,
                args=(values["-WP_ORIGIN-"], values["-WP_DEST-"], replacement_pairs),
                daemon=True
            ).start()
...
# Update the `run_word_processing` function
